...
- How many customers were created in the database?
  It was created five customers.
- Where is the *findAll* method implemented?
  Is is implemented in the CustomerRepository interface and is build on runtime when the autowire annotation is declared.
- Suppose you have more than 1000 products in your database. How would you implement a method for supporting pagination and return pages of 50 products to your frontend?
  Using the static method **of** of the class **PageRequest** with 20 pages and size of 50. 
...
- How many products are returned by the *findByDescriptionContaining* query? Why?
  Two products are returned with the that method because is defined a size inside the static method **of**, however there are 4 product with that description. 
- Which are the collection names where the objects are stored? Where are those names assigned?
  **customer** and **product**. Mongodb creates the collections automatically when is saved some document, so the names of the model are used.
